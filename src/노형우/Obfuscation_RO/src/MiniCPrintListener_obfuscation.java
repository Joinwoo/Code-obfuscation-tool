...
public class MiniCPrintListener_obfuscation extends MiniCBaseListener implements ParseTreeListener {
    ...
    public void exitExpr(MiniCParser.ExprContext ctx) {
        ...
        obfuscation.RanCodeGenerator ranCodeGenerator = new obfuscation.RanCodeGenerator();
        String ranCode = ranCodeGenerator.generateRandomCCode();
        ...
        if (childCount == 1)
            ...
            if (ctx.getChild(0).getText().equals("("))
                ...
                result = "if(false){" + ranCode + "}else{" + ctx.IDENT().getText() + " = " + cTree.get(ctx.expr(0)) + "}";
            else {
